...
au BufHidden <buffer> call s:JSLintClear()
"au ColorScheme <buffer> if line("$") < 400 | call s:JSLint() | endif
"au BufEnter <buffer> call s:GetJSLintMessage()
"au InsertLeave <buffer> call s:JSLint()
"au InsertEnter <buffer> call s:JSLint()
au BufWritePost <buffer> if line("$") < 1500 | call s:JSLint() | endif
...
let s:jslintrc_file = globpath(&rtp, '.jslintrc')
...
function! s:JSLint()
  ...
  if executable('gjslint')
    ...
    let s:cmd = 'gjslint --custom_jsdoc_tags="example,encoding,lastModified,version,file,module,description" ' . iconv( current_file , &enc, "gbk")
    let l:gjslint_output = system(s:cmd)
    let l:gjslint_output = matchstr(l:gjslint_output, 'Line \d\_.*Found \d')
    let b:jslint_output = l:gjslint_output
  else
  ...
  for error in split(b:jslint_output, "\n")
    ...
    let b:parts = matchlist(error, "\\(\\d\\+\\).*:\\([-0-9]\\+\\):\\ \\=\\(.*\\)")
    if !empty(b:parts)
      ...
      let s:matchDict['colNum'] = b:parts[2]
...
if !exists("*s:GetJSLintMessage")
    function s:GetJSLintMessage()
        ...
        if has_key(b:matchedlines, s:cursorPos[1])
            ...
            let s:cursorPos[2] = s:jslintMatch['colNum']
...
if !exists("b:LintJump")
  function! b:LintJump(direction)
...
au FileChangedShellPost <buffer> if search('\r') | exe "%s/\r//g" | endif | call s:JSLintUpdate() | setl autoread< 

noremap <silent> <leader><F4> :setl autoread<cr>:silent !fixjsstyle %<cr>
noremap <silent> <leader><F3> :JSLintUpdate<cr>
noremap <silent> <leader>jj :call b:LintJump("next")<cr>
noremap <silent> <leader>kk :call b:LintJump("prev")<cr>
